            ...
            <div className="flex gap-4 mt-8">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="w-10 h-10 flex items-center justify-center border border-white/10 text-black/60 hover:text-[#C5A861] hover:border-[#C5A861] transition-all duration-500 rounded-full bg-white/5"
                >
            ...
            <ul className="space-y-4">
              {quickLinks.map((link) => (
                <li key={link.path}>
                  <Link
                    to={link.path}
                    className="text-sm text-black/60 hover:text-green transition-all duration-300 flex items-center gap-3 group"
                  >
